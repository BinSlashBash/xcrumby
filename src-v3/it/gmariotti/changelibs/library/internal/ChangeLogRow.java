...
public class ChangeLogRow {
    ...
    public String getChangeText(Context context) {
        ...
        String prefix = UnsupportedUrlFragment.DISPLAY_NAME;
        switch (this.type) {
            case BUGFIX /*1*/:
                prefix = context.getResources().getString(C0659R.string.changelog_row_prefix_bug).replaceAll("\\[", "<").replaceAll("\\]", ">");
                break;
            case IMPROVEMENT /*2*/:
                prefix = context.getResources().getString(C0659R.string.changelog_row_prefix_improvement).replaceAll("\\[", "<").replaceAll("\\]", ">");
                break;
        }
        return prefix + MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR + this.changeText;
    }
