...
public class SlidingPaneLayout extends ViewGroup {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (this.mFirstLayout) {
            float f = (this.mCanSlide && this.mPreservedOpenState) ? GalleryViewer.PROGRESS_COMPLETED : 0.0f;
            this.mSlideOffset = f;
        }
        for (i = 0; i < childCount; i++) {
            View child = getChildAt(i);
            if (child.getVisibility() != 8) {
                ...
                if (lp.slideable) {
                    ...
                    offset = (int) ((GalleryViewer.PROGRESS_COMPLETED - this.mSlideOffset) * ((float) this.mParallaxBy));
                    xStart = nextXStart;
                }
    ...
    private boolean openPane(View pane, int initialVelocity) {
        if (!this.mFirstLayout && !smoothSlideTo(GalleryViewer.PROGRESS_COMPLETED, initialVelocity)) {
            return false;
        }
    ...
    public boolean isOpen() {
        return !this.mCanSlide || this.mSlideOffset == GalleryViewer.PROGRESS_COMPLETED;
    }
    ...
    private void parallaxOtherViews(float slideOffset) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (v != this.mSlideableView) {
                int oldOffset = (int) ((GalleryViewer.PROGRESS_COMPLETED - this.mParallaxOffset) * ((float) this.mParallaxBy));
                this.mParallaxOffset = slideOffset;
                v.offsetLeftAndRight(oldOffset - ((int) ((GalleryViewer.PROGRESS_COMPLETED - slideOffset) * ((float) this.mParallaxBy))));
                if (dimViews) {
                    dimChildView(v, GalleryViewer.PROGRESS_COMPLETED - this.mParallaxOffset, this.mCoveredFadeColor);
                }
            }
        }
    }
