...
public class ViewDragHelper {
    ...
    static class C00501 implements Interpolator {
        C00501() {
        }
    ...
    public static ViewDragHelper create(ViewGroup forParent, float sensitivity, Callback cb) {
        ...
        helper.mTouchSlop = (int) (((float) helper.mTouchSlop) * (GalleryViewer.PROGRESS_COMPLETED / sensitivity));
        return helper;
    }

    private ViewDragHelper(Context context, ViewGroup forParent, Callback cb) {
        this.mActivePointerId = INVALID_POINTER;
        this.mSetIdleRunnable = new C00512();
        if (forParent == null) {
    ...
    private int computeAxisDuration(int delta, int velocity, int motionRange) {
        ...
        float distance = ((float) halfWidth) + (((float) halfWidth) * distanceInfluenceForSnapDuration(Math.min(GalleryViewer.PROGRESS_COMPLETED, ((float) Math.abs(delta)) / ((float) width))));
        velocity = Math.abs(velocity);
        if (velocity > 0) {
            duration = Math.round(1000.0f * Math.abs(distance / ((float) velocity))) * EDGE_TOP;
        } else {
            duration = (int) (((((float) Math.abs(delta)) / ((float) motionRange)) + GalleryViewer.PROGRESS_COMPLETED) * 256.0f);
        }
