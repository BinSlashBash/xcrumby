...
public class FileProvider extends ContentProvider {
    ...
    static class SimplePathStrategy implements PathStrategy {
        ...
        public Uri getUriForFile(File file) {
            try {
                ...
                if (rootPath.endsWith(DeviceFragment.REGEX_BASE)) {
                    path = path.substring(rootPath.length());
                } else {
                    path = path.substring(rootPath.length() + 1);
                }
                return new Builder().scheme("content").authority(this.mAuthority).encodedPath(Uri.encode((String) mostSpecific.getKey()) + '/' + Uri.encode(path, DeviceFragment.REGEX_BASE)).build();
            } catch (IOException e) {
    ...
    static {
        ...
        DEVICE_ROOT = new File(DeviceFragment.REGEX_BASE);
        sCache = new HashMap();
    }
