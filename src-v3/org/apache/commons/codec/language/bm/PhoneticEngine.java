...
public class PhoneticEngine {
    ...
    static final class PhonemeBuilder {
        ...
        public static PhonemeBuilder empty(LanguageSet languages) {
            return new PhonemeBuilder(new Phoneme(UnsupportedUrlFragment.DISPLAY_NAME, languages));
        }
    ...
    private static final class RulesApplication {
        ...
        public RulesApplication invoke() {
            this.found = false;
            int patternLength = 1;
            List<Rule> rules = (List) this.finalRules.get(this.input.subSequence(this.f58i, this.f58i + 1));
            if (rules != null) {
                for (Rule rule : rules) {
                    patternLength = rule.getPattern().length();
                    if (rule.patternAndContextMatches(this.input, this.f58i)) {
    ...
    public String encode(String input, LanguageSet languageSet) {
        ...
        if (this.nameType == NameType.GENERIC) {
            ...
            if (input.length() < 2 || !input.substring(0, 2).equals("d'")) {
                for (String l : (Set) NAME_PREFIXES.get(this.nameType)) {
                    if (input.startsWith(l + MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR)) {
        ...
        switch (C06821.$SwitchMap$org$apache$commons$codec$language$bm$NameType[this.nameType.ordinal()]) {
            case Std.STD_FILE /*1*/:
        ...
        if (this.concat) {
            input = join(words2, MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR);
