...
public class TextListener extends RunListener {
    ...
    protected void printFailures(Result result) {
        ...
        if (failures.size() != 0) {
            ...
            for (Failure each : failures) {
                int i2 = i + 1;
                printFailure(each, UnsupportedUrlFragment.DISPLAY_NAME + i);
                i = i2;
            }
        }
    }
    ...
    protected void printFooter(Result result) {
        if (result.wasSuccessful()) {
            getWriter().println();
            getWriter().print("OK");
            getWriter().println(" (" + result.getRunCount() + " test" + (result.getRunCount() == 1 ? UnsupportedUrlFragment.DISPLAY_NAME : DanbooruImageFragment.BREADCRUMB_NAME) + ")");
        } else {
            getWriter().println();
